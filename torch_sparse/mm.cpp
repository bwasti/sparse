...
static auto registry0 = torch::RegisterOperators()
	.op("sparse::bsmm",
  torch::RegisterOperators::options().kernel(c10::TensorTypeId::CUDATensorId,
		[](at::Tensor X, at::Tensor W, int64_t out, at::Tensor lut, int64_t block_size, int64_t num_segs, int64_t max_seg_len) -> at::Tensor {
    TORCH_CHECK(X.dim() == 2);
    if (!X.is_contiguous()) {
	...
	.op("sparse::bsmm_dx",
  torch::RegisterOperators::options().kernel(c10::TensorTypeId::CUDATensorId,
		[](at::Tensor dY, at::Tensor W, int64_t out, at::Tensor lut, int64_t block_size, int64_t num_segs, int64_t max_seg_len) -> at::Tensor {
	...
	.op("sparse::bsmm_dw",
  torch::RegisterOperators::options().kernel(c10::TensorTypeId::CUDATensorId,
		[](at::Tensor X, at::Tensor dY, at::Tensor lut, int64_t block_size) -> at::Tensor {
    if (!dY.is_contiguous()) {
      //std::cerr << "non-contiguous dY, likely slowdown\n";
      dY = dY.contiguous();
    }
